...
from .enum import CloseBoardRareState

class CloseBoardRareStrategy(BaseStrategy):
    def __init__(self, serial):
        self.serial = serial

    def check(self) -> bool:
        return self._detect_state() != CloseBoardRareState.UNKNOWN

    def _detect_state(self) -> CloseBoardRareState:
        if exist(self.serial, MainView.TO_DOWNLOAD_TEXT.value, threshold=0.9): 
            return CloseBoardRareState.TO_DOWNLOAD
        
        return CloseBoardRareState.UNKNOWN
    
    def proccess(self):
        state = self._detect_state()

        if state == CloseBoardRareState.TO_DOWNLOAD:
