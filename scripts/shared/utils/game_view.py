...
def close_board(serial, attempts=25):
    ...
    for _ in range(attempts):
        if wait_click(serial, MainView.BOARD_DONT_SHOW.value, timeout=3.0):
            wait_click(serial, MainView.CLOSE_BOARD.value, timeout=3.0)
            num = 0
            continue
        if wait_click(serial, MainView.CLOSE_BOARD.value, timeout=3.0):
            num = 0
            continue
        elif wait(serial, MainView.BOARD_END.value, timeout=3.0, threshold=0.9):
            wait_click(serial, Confirm.SMALL.value, wait_time=2.5)
            num = 0
            continue
        else:
            num += 1
